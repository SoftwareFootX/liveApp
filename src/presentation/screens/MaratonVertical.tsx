...
const MaratonVertical = () => {
  ...
  const calculatePelvis = (
    ...
    const lx = rightHip.x * canvasWidth;
    const ly = rightHip.y * canvasHeight;
    const rx = leftHip.x * canvasWidth;
    const ry = leftHip.y * canvasHeight;

    // Diferencias
    const dx = ry - ly; // distancia horizontal entre caderas
    const dy = rx - lx; // diferencia vertical (positivo si derecha está más abajo)
  ...
  const calculateTorsoTilt = (
    ...
    const lx = leftShoulder.x * canvasHeight;
    const ly = leftShoulder.y * canvasWidth;
    const rx = rightShoulder.x * canvasHeight;
    const ry = rightShoulder.y * canvasWidth;
    ...
    pose.onResults((results) => {
      ...
      if (results.poseLandmarks) {
        ...
        angulos.forEach(([a, b, c]) => {
          ...
          drawRotatedText(ctx, `${angle}º`, bx + 5, by - 5, -Math.PI / 2);
        ...
        if (side === "both") {
          ...
          // Hombro Der (rotado 90º)
          drawRotatedText(
            ctx,
            `Hombro Der: ${inclinacionDeToraXDer.toFixed(1)}º`,
            lxTorax - 70,
            lyTorax - 10,
            -Math.PI / 2
          );

          // Hombro Izq (rotado 90º)

          drawRotatedText(
            ctx,
            `Hombro Izq: ${toraxAngleIzq.toFixed(1)}º`,
            lxTorax + 30,
            lyTorax - 10,
            -Math.PI / 2
          );
        ...
        <div className="flex justify-center items-center relative">
          <div className="h-screen w-full relative overflow-hidden bg-black z-5 flex justify-center items-center">
            <video
              ...
              playsInline
              muted
              className="h-screen w-auto transform rotate-90 object-cover"
            />
            <canvas
              ref={canvasRef}
              className="absolute top-0 left-1/2 -translate-x-1/2 h-screen w-auto z-10 pointer-events-none transform rotate-90 object-cover"
            />
          </div>
        </div>
